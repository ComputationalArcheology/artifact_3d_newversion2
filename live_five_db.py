...
class LiveFiveViewsWidget(QWidget):
    ...
    def _update_scale_bar(self):
        if self.lbl_scale is None or self._mesh is None: return
        try:
            mc_pl = self._plotters.get("MC")
            if mc_pl:
                ps = mc_pl.camera.GetParallelScale()
                height_px = mc_pl.height()
                world_height = ps * 2.0
                units_per_pixel = world_height / height_px if height_px > 0 else 0
                bar_px = 100
                bar_units = bar_px * units_per_pixel
                self.lbl_scale.setText(f"Scale: {bar_units:.2f} units / {bar_px} px")
        except:
