...
# ============================================================
#  A) עזרי פריסה (Layout) בסגנון MATLAB
# ============================================================
@dataclass
class NRect:
    x: float
...
class MatlabLiveViewsCanvas(QWidget):
    ...
    def _layout_matlab_like(self) -> None:
        ...
        # תמיד משתמשים במיקומים הבסיסיים - ריבועים שווים בגודלם
...
class LiveViewsWindow(QWidget):
    ...
    def __init__(self, mesh_aligned: pv.PolyData, parent=None):
        ...
        # --- שלב 1: הגדרת מיקומי מצלמה וחישוב ה-Scale הנדרש לכל מבט ---
        temp_fits = {}
        ...
        # --- שלב 2: מציאת ה-Scale המקסימלי והחלתו על כל התצוגות באופן אחיד ---
        global_fit = max(temp_fits.values()) if temp_fits else 1.0

        for key, p in self.plotters.items():
            self._set_parallel_scale(p, global_fit)
            self._base_scales[key] = global_fit
            self._last_fit_by_key[key] = global_fit
    ...
    def _on_canvas_layout_changed(self) -> None:
        ...
        try:
            ...
            for key, p in self.plotters.items():
                ...
                temp_fits[key] = fit_new

            # מחשב זום גלובלי משותף בכל שינוי גודל חלון
            global_fit = max(temp_fits.values()) if temp_fits else 1.0
    ...
    def _on_any_camera_modified(self, caller, event):
        """
        סנכרון תנועה בין המצלמות.
        מחשב את השינוי (דלתא) מהמצלמה הפעילה ומחיל אותו על שאר המבטים ביחס לבסיס שלהם.
        """
        ...
        F_base = self._frame_from_base(self._base_angles[key_src])
        fwd_cur, up_cur = self._read_fwd_up(src_plotter)
        F_cur = self._frame_from_fwd_up(fwd_cur, up_cur)

        R_delta = F_cur @ F_base.T

        # קוראים את הזום מהמצלמה שהמשתמש כרגע נוגע בה
        scale_now = self._get_parallel_scale(src_plotter)
        self._last_fit_by_key[key_src] = scale_now
        ...
        try:
            for key, p in self.plotters.items():
                ...
                # כולם מקבלים את אותו ה-Scale בדיוק
                self._set_parallel_scale(p, scale_now)
                self._last_fit_by_key[key] = scale_now
    ...
    # אופציונלי: שמירת קולאז' של כל המבטים
