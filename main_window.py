...
from artifact_app.gui.angle_between_surfaces_window import AngleBetweenSurfacesWindow
...
class MainWindow(QMainWindow):
    def __init__(self):
        ...
        self._cut_line_on: bool = False
        self._cut_z: float = 0.0
        ...
        self.crop_win = None
        self._cached_measurements = None  # ישמור את הנתונים המחושבים לשימוש עתידי
        self._measurements_worker = None  # הפניה ל-Thread שרץ ברקע

        central = QWidget(self)
        central.setObjectName("CentralWidget")  # התאמה ל-QSS
        ...
        self.side_left.setObjectName("SidePanelLeft")  # התאמה ל-QSS
        self.side_left.setFrameShape(QFrame.NoFrame)
        ...
        # --- תצוגת מידות בפאנל השמאלי ---
        self.lbl_dim_title = QLabel("Object Dimensions", self.side_left)
        self.lbl_dim_title.setObjectName("DimTitle")

        self.lbl_dim_x = QLabel("X (Width): --", self.side_left)
        self.lbl_dim_y = QLabel("Y (Depth): --", self.side_left)
        self.lbl_dim_z = QLabel("Z (Height): --", self.side_left)

        self.lbl_dim_x.setObjectName("DimValue")
        self.lbl_dim_y.setObjectName("DimValue")
        self.lbl_dim_z.setObjectName("DimValue")

        self.side_left_layout.addWidget(self.lbl_dim_title)
        self.side_left_layout.addWidget(self.lbl_dim_x)
        self.side_left_layout.addWidget(self.lbl_dim_y)
        self.side_left_layout.addWidget(self.lbl_dim_z)
        self.side_left_layout.addStretch(1)  # דוחף הכל למעלה

        self.lbl_dim_title.setVisible(False)
        self.lbl_dim_x.setVisible(False)
        self.lbl_dim_y.setVisible(False)
        self.lbl_dim_z.setVisible(False)
        ...
        self.positioning_mode = "Normal Positioning"

        self.label_positioning = QLabel(f"[{self.positioning_mode}]", content)
        self.label_positioning.setAlignment(Qt.AlignCenter)
        self.label_positioning.setStyleSheet("font-weight: bold; color: #555; font-size: 12px;")
        self.label_positioning.setVisible(False)
        content_v.addWidget(self.label_positioning)

        self.label_filename = QLabel("", content)
        self.label_filename.setObjectName("LabelFilename")  # התאמה ל-QSS
        self.label_filename.setAlignment(Qt.AlignCenter)
        content_v.addWidget(self.label_filename)

        self.views_box = QFrame(content)
        self.views_box.setObjectName("ViewsBox")  # התאמה ל-QSS
        ...
        side = QFrame(central)
        side.setObjectName("SidePanelRight")  # התאמה ל-QSS
        ...
        #self.btn_create_cut.setObjectName("btnCreateCut")
        #self.btn_create_cut.clicked.connect(self.open_cross_section_window)

        #self.btn_abs = ArtifactButton("Angle between surfaces", side)
        #self.btn_abs.setObjectName("btnAbs")
        #self.btn_abs.clicked.connect(self.open_angle_between_surfaces_window)
    ...
    def _create_menubar(self):
        ...
        # כל עיצוב ה-QSS הוסר מכאן. הוא מנוהל ב-style.qss
        ...
        act_last_position = QAction("last manual positioning", self)
        act_last_position.triggered.connect(self.apply_last_manual_positioning)  # הוספנו חיבור
        m_position.addAction(act_last_position)

        act_normal_position = QAction("normal positioning", self)
        act_normal_position.triggered.connect(self.apply_normal_positioning)  # הוספנו חיבור
        m_position.addAction(act_normal_position)
        ...
        act_crop = QAction("Crop Object", self)
        act_crop.triggered.connect(self.open_crop_window)
        m_functions.addAction(act_crop)

        act_cut = QAction("Create cut", self)
        act_cut.triggered.connect(self.open_cross_section_window)
        m_functions.addAction(act_cut)

        act_abs = QAction("Angle between surfaces", self)
        act_abs.triggered.connect(self.open_angle_between_surfaces_window)
        m_functions.addAction(act_abs)
    ...
    def open_crop_window(self):
        ...
        # הוספנו את העברת משתנה המצב לחלון החדש
    ...
    def _world_z_to_pixel_y(self, z_world: float, meta: dict) -> int:
        """ממיר נקודת גובה Z במרחב התלת-ממדי למיקום Y בפיקסלים על ה-Pixmap"""
        cx, cy, cz = map(float, meta.get("center", (0.0, 0.0, 0.0)))
        ps = float(meta.get("parallel_scale", 1.0))
        w, h = map(int, meta.get("window_size", (1, 1)))

        # וקטור ה-UP קובע מהו הכיוון האנכי (Y) בתמונה הדו-ממדית
        up = np.asarray(meta.get("up", [0, 1, 0]), float)
        up /= (np.linalg.norm(up) + 1e-12)

        # המרחק האנכי מהמרכז הוא ההיטל של הפרש הגבהים על וקטור ה-UP
        d = np.array([0.0, 0.0, z_world - cz], float)
        yw = float(np.dot(d, up))

        # חישוב הפיקסל (כולל היפוך הציר כי QPixmap מתחיל מ-0 למעלה)
        v = int((yw / (2 * ps) + 0.5) * h)
        return h - v

        # --- הפונקציה הישנה (מבוססת Bounding Box) נשמרת כגיבוי ---
        # def update_dimensions_panel(self, mesh: pv.PolyData | None) -> None:
        #     if mesh is None:
        #         self.lbl_dim_title.setVisible(False)
        #         self.lbl_dim_x.setVisible(False)
        #         self.lbl_dim_y.setVisible(False)
        #         self.lbl_dim_z.setVisible(False)
        #         return
        #     dx, dy, dz = bbox_deltas_from_bounds(mesh.bounds)
        #     self.lbl_dim_x.setText(f"X (Width): {dx:.2f} mm")
        #     self.lbl_dim_y.setText(f"Y (Depth): {dy:.2f} mm")
        #     self.lbl_dim_z.setText(f"Z (Height): {dz:.2f} mm")
        #     self.lbl_dim_title.setVisible(True)
        #     self.lbl_dim_x.setVisible(True)
        #     self.lbl_dim_y.setVisible(True)
        #     self.lbl_dim_z.setVisible(True)
        # ---------------------------------------------------------
    ...
    def update_dimensions_panel(self, mesh: pv.PolyData | None) -> None:
            ...
            # 1. מציגים את התוויות עם אנימציה של טעינה
            self.lbl_dim_x.setText("X (Width): Calculating...")
            self.lbl_dim_y.setText("Y (Depth): Calculating...")
            self.lbl_dim_z.setText("Z (Height): Calculating...")
            ...
            # 2. עוצרים את תהליך הרקע הקודם אם משום מה הוא עדיין רץ
            if getattr(self, "_measurements_worker", None) is not None:
                self._measurements_worker.terminate()
                self._measurements_worker.wait()

            # 3. מכינים נתונים ושולחים לחישוב ברקע
            mc_meta = getattr(self, "_last_meta", {}).get("MC", {})

            self._measurements_worker = MeasurementsWorker(mesh, mc_meta)
            self._measurements_worker.measurements_ready.connect(self._on_measurements_ready)
            self._measurements_worker.start()

    def _on_measurements_ready(self, measurements: Any) -> None:
            """נקראת אוטומטית כשהחישוב מסיים לרוץ ברקע"""
            if measurements is None:
                self.lbl_dim_x.setText("X (Width): Error")
                self.lbl_dim_y.setText("Y (Depth): Error")
                self.lbl_dim_z.setText("Z (Height): Error")
                return

            # שומרים את האובייקט בזיכרון של המחלקה הראשית, כדי שתוכל
            # להשתמש בו ישירות בחלונות אחרים כמו CrossSection או ProcessObject
            self._cached_measurements = measurements

            # שולפים את המיתרים המקסימליים האמיתיים מתוך האובייקט המחושב
            dx = measurements.profile_xz.max_chord
            dy = measurements.profile_yz.max_chord

            # שולפים את הגובה מנתוני המטלב. גיבוי - לוקחים מה-Bbox אם משהו השתבש.
            if measurements.profile_xz.matlab_data and measurements.profile_xz.matlab_data.cordh > 0:
                dz = measurements.profile_xz.matlab_data.cordh
            else:
                dz = measurements.dz_world

            self.lbl_dim_x.setText(f"X (Width): {dx:.2f} mm")
            self.lbl_dim_y.setText(f"Y (Depth): {dy:.2f} mm")
            self.lbl_dim_z.setText(f"Z (Height): {dz:.2f} mm")

    def _apply_cut_line_overlay(self) -> None:
        """מרנדר מחדש את תצוגת MC ומשלב את כל השכבות הפעילות (BBox, חיתוך, מרכז כובד)"""
        ...
        # 1. מתחילים מהתמונה הבסיסית הנקייה
        pix_drawn = QPixmap(base_pix)

        # 2. ציור תיבת התיחום והרקע האפור (אם דלוק)
        if getattr(self, "_bbox_on", False) and getattr(self, "_bbox_cache", None):
            pix_drawn = self._draw_bbox_rulers_simple(
                pix_drawn,
                bbox_px=self._bbox_cache["bbox_px"],
                dx_mm=self._bbox_cache["bbox_span_mm"][0],
                dz_mm=self._bbox_cache["bbox_span_mm"][1]
            )

        # 3. ציור קווי החיתוך (אם דלוק)
        if getattr(self, "_cut_line_on", False):
            painter = QPainter(pix_drawn)
            pen = QPen(Qt.black, 2)
            ...
            if bbox_px is None:
                obj_left, obj_right = 0, base_pix.width()
            else:
                obj_left, obj_right, _, _ = bbox_px

            v_px = self._world_z_to_pixel_y(self._cut_z, mc_meta)
            painter.drawLine(0, v_px, obj_left, v_px)
            painter.drawLine(obj_right, v_px, base_pix.width(), v_px)
            painter.end()

        # 4. ציור מרכז הכובד (אם דלוק)
        if getattr(self, "_com_on", False):
            try:
                com, _ = compute_area_weighted_centroid(self._last_mesh_aligned)
                u, v = self._world_to_mr_pixels(com, mc_meta, flip_v=True)
                pix_drawn = self._draw_crosshair_on_pixmap(pix_drawn, int(u), int(v), radius=5)
            except Exception:
                pass

        # עדכון ה-Label עם התמונה הסופית שכוללת הכל
        lbl = self.labels.get("MC")
        if lbl is not None and not getattr(lbl, "_logically_hidden", False):
            lbl.setPixmap(pix_drawn)
            self._last_pixmaps["MC"] = pix_drawn
    def on_open_folder(self):
        ...
        supported_extensions = {".ply", ".stl", ".obj", ".vtk", ".vtp"}
        files = [f for f in base_dir.iterdir() if f.suffix.lower() in supported_extensions]
    ...
    def run_batch_processing(self, files, output_dir):
        ...
        # 1. יצירת חותמת זמן ותיקיית אב ייעודית לייצוא הנוכחי
        timestamp = datetime.now().strftime('%Y%m%d_%H%M')
        batch_export_dir = output_dir / f"Batch_Export_{timestamp}"
        batch_export_dir.mkdir(parents=True, exist_ok=True)

        # 2. ניתוב תתי-התיקיות לתוך תיקיית הייצוא החדשה
        dir_main = batch_export_dir / "Screenshots_MainView"
        dir_graphs = batch_export_dir / "Screenshots_Graphs"
        ...
        if all_objects_data:
            # 3. ניתוב קובץ האקסל לאותה תיקייה
            excel_path = batch_export_dir / f"Batch_Summary_{timestamp}.xlsx"
            ...
            QMessageBox.information(
                ...
                f"Images and report saved to:\n{batch_export_dir}" # עדכון הודעת הסיום
            )
    ...
    def on_crop_applied(self, new_mesh):
        ...
        self.update_dimensions_panel(self._last_mesh_aligned)
    ...
    def open_manual_measurements(self) -> None:
        mesh = getattr(self, "_last_mesh_aligned", None)
        if mesh is None:
            QMessageBox.warning(self, "No object", "No object loaded. Load a model to proceed")
            return
    ...
    def load_mesh_and_show(self, path: str) -> None:
        try:
            ...
            if path.lower().endswith(".wrl"):
                self._set_status(f"Converting WRL file: {os.path.basename(path)}...")
                # ברגע שזה מסיים, path מצביע לקובץ ה-PLY החדש
                path = convert_wrl_to_ply(path)
                self._set_status("Conversion successful. Loading mesh...")
            # ------------------------

            # מכאן והלאה PyVista עובדת כרגיל, היא לא יודעת שזה היה פעם WRL
            mesh = pv.read(path)

            if not isinstance(mesh, pv.PolyData) or mesh.n_points == 0:
                raise ValueError(f"הקובץ {os.path.basename(path)} ריק או אינו מכיל משולשים נתמכים.")

            # שומרים עותק מקורי ללא ניקוי אוטומטי כדי לחסוך זמן טעינה במודלים כבדים
            self._last_mesh_raw = mesh.copy()

            # מנגנון הגיבוי: מנסים ליישר את המודל כפי שהוא.
            # אם הוא פגום והמתמטיקה קורסת, נתפוס את השגיאה, ננקה אותו, וננסה שוב.
            try:
                mesh_aligned, _Ttr = align_mesh(mesh, method="matlab")
            except Exception as e:
                self._set_status(f"Alignment failed, model might be dirty. Cleaning... ({e})")
                # מפעילים את פעולת הניקוי הכבדה רק כשאין ברירה
                mesh = mesh.clean(inplace=False)
                self._last_mesh_raw = mesh.copy() # מעדכנים את הגיבוי למודל הנקי
                mesh_aligned, _Ttr = align_mesh(mesh, method="matlab")
            ...
            self._auto_aligned_mesh = mesh_aligned.copy(deep=True)
            self._last_manual_mesh = None

            dx, dy, dz = bbox_deltas_from_bounds(mesh_aligned.bounds)
            self.five_views_canvas.set_bbox(dx, dy, dz)

            # === התוספת: איפוס כל מצבי התצוגה לפני הרינדור מחדש ===
            self._com_on = False
            self._pixmaps_before_com = {}

            self._cut_line_on = False
            self._cut_z = 0.0
            self._last_cut_pixmap = None

            # ניקוי משבצת החתך (המשבצת השמאלית התחתונה)
            cut_lbl = self.labels.get("CUT_RESULT")
            if cut_lbl is not None:
                cut_lbl.clear()

            # ביטול מצבי הסתרה (למקרה שהסתרת את אחת התצוגות באובייקט הקודם)
            ...
            self.positioning_mode = "Normal Positioning"
            self.label_positioning.setText(f"[{self.positioning_mode}]")
            self.label_positioning.setVisible(True)
    ...
    def _on_resize_settled(self) -> None:
        ...
        # עדכון גודל תמונת החתך (אם היא קיימת) תוך כדי גרירת החלון
        if hasattr(self, '_last_cut_pixmap') and self._last_cut_pixmap is not None:
            lbl = self.five_views_canvas.labels.get("CUT_RESULT")
            if lbl:
                scaled_pix = self._last_cut_pixmap.scaled(lbl.size(), Qt.KeepAspectRatio, Qt.SmoothTransformation)
    ...
    def _on_refresh_clicked(self) -> None:
        ...
        # === התוספת לתיקון הסינכרון של ה-GUI ===
        # שחרור כל הכפתורים שהם Checkable כדי שיתאימו למצב התצוגה המאופס
        self.btn_hide_front.setChecked(False)
        self.btn_hide_right.setChecked(False)
        self.btn_hide_back.setChecked(False)
        self.btn_hide_top.setChecked(False)
        self.btn_hide_bottom.setChecked(False)
        self.btn_hide_title.setChecked(False)
        self.btn_hide_scale.setChecked(False)

        # View Control ייסגר בעת ה-Restore
        self.btn_view_control.setChecked(False)
        self.view_controls_box.setVisible(False)
        # =======================================
        ...
        # ... (המשך הפונקציה המקורית שלך נשאר ללא שינוי)
        self._last_cut_pixmap = None
        cut_lbl = self.labels.get("CUT_RESULT")
        if cut_lbl is not None:
            cut_lbl.clear()
        ...
        # מוודאים שאם לחצנו רסטור ידני, נחזור לנורמל באמת
        if getattr(self, "_is_applying_saved_mode", False):
            self._is_applying_saved_mode = False
        else:
            self.positioning_mode = "Normal Positioning"
            if getattr(self, "_auto_aligned_mesh", None) is not None:
                self._original_aligned_mesh = self._auto_aligned_mesh.copy(deep=True)
    ...
    def _on_build_bbox_clicked(self) -> None:
        if getattr(self, "_bbox_on", False):
            ...
            # קריאה לפונקציית העל שתרכיב את השכבות מחדש (בלעדי ה-BBox כי כיבינו אותו)
            self._apply_cut_line_overlay()
            return
        ...
        try:
            ...
            # קריאה לפונקציית העל שתרכיב את כל השכבות (כולל ה-BBox שהרגע הדלקנו)
            self._apply_cut_line_overlay()
    ...
    def open_live_views(self):
        ...
        # לוקחים את המש המקורי הנקי כדי שהחלון יתחיל מנקודת אפס,
        # אבל מבלי לדרוס עדיין את התצוגה הראשית!
        base_mesh = getattr(self, "_original_aligned_mesh", getattr(self, "_last_mesh_aligned", None))
        ...
        # שולחים לחלון עותק של המש המקורי בלבד
        self.live_views_win = LiveViewsWindow(base_mesh.copy(deep=True), parent=None)
    ...
    def _on_live_view_applied(self, params: dict) -> None:
        ...
        try:
            ...
            if rotated_vertices is not None:
                ...
                self._last_manual_mesh = mesh_rot.copy(deep=True)


                self.positioning_mode = "Manual Positioning"
                self.label_positioning.setText(f"[{self.positioning_mode}]")
    ...
    def open_cross_section_window(self) -> None:
        ...
        # <--- חיבור האיתות (Signal) כדי שנוכל לתפוס את התמונה כשהיא מוכנה --->
        if hasattr(self.cross_section_win, 'cut_confirmed_signal'):
            self.cross_section_win.cut_confirmed_signal.connect(self.display_cut_result)
    ...
    def display_cut_result(self, pixmap: QPixmap, contours=None, is_looking_up=False, cut_z: float = None):
        lbl = self.five_views_canvas.labels.get("CUT_RESULT")
        if not lbl: return

        lbl.setStyleSheet("background: transparent; border: none;")

        # גיבוי למקרה חירום: אם אין קונטורים נשתמש בתמונה
        if not contours:
            self._last_cut_pixmap = pixmap  # שמירה כדי שלא יעלם בהקטנת חלון
            scaled_pix = pixmap.scaled(lbl.size(), Qt.KeepAspectRatio, Qt.SmoothTransformation)
            lbl.setPixmap(scaled_pix)
            lbl.show()
            lbl.raise_()
            return
        ...
        # יצירת תמונה חלקה ונקייה בגודל המשבצת
        new_pix = QPixmap(w, h)
        new_pix.fill(QColor("white"))

        tl_meta = self._last_meta.get("TL", {})
        if not tl_meta:
            tl_meta = self._last_meta.get("MC", {})

        scale = tl_meta.get("world_per_px", 1.0)

        # --- התיקון הקריטי: התאמת קנה המידה מ-Supersampling לרזולוציית המשבצת ---
        w_r = tl_meta.get("render_window_size", [w, h])[0]
        w_t = tl_meta.get("window_size", [w, h])[0]
        ss_ratio = float(w_r) / max(1, float(w_t))
        scale = scale * ss_ratio
        if scale <= 0: scale = 1.0
        # -------------------------------------------------------------------------

        center = tl_meta.get("center", [0, 0, 0])
        ...
        for cnt in contours:
            ...
            for pt in cnt:
                if is_looking_up:
                    u = w / 2.0 - (pt[0] - cx) / scale
                else:
                    u = (pt[0] - cx) / scale + w / 2.0

                    # החזרנו למינוס כדי שציר ה-Y יצביע כלפי מעלה (תואם למטלב ולחלון המדידות)
                v = h / 2.0 - (pt[1] - cy) / scale
                poly.append(QPointF(u, v))
            painter.drawPolygon(poly)

        painter.end()

        self._last_cut_pixmap = new_pix  # שמירה כדי שהחתך לא יעלם בהזזת החלון הראשי
        ...
        if cut_z is not None:
            self._cut_z = cut_z
            self._cut_line_on = True
            self._apply_cut_line_overlay()

    def open_angle_between_surfaces_window(self) -> None:
        mesh = getattr(self, "_last_mesh_aligned", None)
        if mesh is None:
            QMessageBox.warning(self, "No object", "No object loaded. Load a model to proceed")
            return

        # כדי שהחלון לא ייסגר כשהפונקציה מסתיימת, נשמור אותו כמשתנה במחלקה
        self.abs_win = AngleBetweenSurfacesWindow(
            mesh,
            parent=None,
            positioning_mode=self.positioning_mode
        )

        self.abs_win.setWindowFlags(
            Qt.Window
            | Qt.WindowMinimizeButtonHint
            | Qt.WindowMaximizeButtonHint
            | Qt.WindowCloseButtonHint
    ...
    def apply_normal_positioning(self):
        """חזרה להעמדה האוטומטית המקורית"""
        if getattr(self, "_auto_aligned_mesh", None) is None:
            QMessageBox.information(self, "Info", "No normal positioning is currently loaded.")
            return

        self._is_applying_saved_mode = True
        self._original_aligned_mesh = self._auto_aligned_mesh.copy(deep=True)
        self.positioning_mode = "Normal Positioning"

        # קוראים לריפרש שעושה עבורנו את העבודה הקשה של עדכון כל התצוגות
        self._on_refresh_clicked()
        self._set_status("Reverted to Normal Positioning.")

    def apply_last_manual_positioning(self):
        """חזרה להעמדה הידנית האחרונה שבוצעה"""
        if getattr(self, "_last_manual_mesh", None) is None:
            QMessageBox.information(self, "Info", "No manual positioning has been saved yet.")
            return

        self._is_applying_saved_mode = True
        self._original_aligned_mesh = self._last_manual_mesh.copy(deep=True)
        self.positioning_mode = "Manual Positioning"

        # מרפרשים
        self._on_refresh_clicked()
        self._set_status("Reverted to Last Manual Positioning.")
def convert_wrl_to_ply(wrl_path: str) -> str:
    """
    ממיר קובץ WRL ל-PLY תוך עקיפת באגים של ספריות C++ עם נתיבים בעברית.
    """
    import pymeshlab
    import os
    import tempfile
    ...
    try:
        # 1. יצירת סביבת עבודה "סטרילית" בתיקייה הזמנית של Windows
        temp_dir = tempfile.gettempdir()

        # שימוש בשמות קבצים באנגלית בלבד עבור תחנת המעבר
        temp_wrl_path = os.path.join(temp_dir, "temp_artifact_input.wrl")
        temp_ply_path = os.path.join(temp_dir, "temp_artifact_output.ply")

        # 2. העתקת קובץ המקור לתיקייה הזמנית (פייתון מטפל בעברית בלי בעיה)
        shutil.copy2(wrl_path, temp_wrl_path)

        # 3. PyMeshLab מבצע את ההמרה בסביבה ללא עברית
        ms = pymeshlab.MeshSet()
        ms.load_new_mesh(temp_wrl_path)
        ms.save_current_mesh(temp_ply_path)

        # 4. הכנת הנתיב הסופי והעתקת התוצאה חזרה לתיקייה המקורית
        base_name = os.path.splitext(os.path.basename(wrl_path))[0]
        final_ply_path = os.path.join(os.path.dirname(wrl_path), f"{base_name}_converted.ply")

        shutil.copy2(temp_ply_path, final_ply_path)

        # 5. ניקוי התיקייה הזמנית (לא נורא אם נכשל)
        try:
        ...
        raise RuntimeError(f"PyMeshLab failed to convert WRL file.\nError details: {str(e)}")
...
# --- תוספת: מחלקה להרצת המדידות ברקע ---
class MeasurementsWorker(QThread):
    measurements_ready = Signal(object) # ישדר את אובייקט ה-ProcessObjectMeasurements

    def __init__(self, mesh: pv.PolyData, mc_meta: dict):
        super().__init__()
        self.mesh = mesh
        self.mc_meta = mc_meta

    def run(self):
        try:
            # מריצים את הפונקציה הכבדה. נשתמש במידות מסך דמי (500x500)
            # כי האלגוריתם המתמטי של המיתרים מסתמך רק על ה-mesh.
            ...
            self.measurements_ready.emit(m)
        except Exception as e:
            print(f"MeasurementsWorker error: {e}")
            self.measurements_ready.emit(None)
# ---------------------------------------