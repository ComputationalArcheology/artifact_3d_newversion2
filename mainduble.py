...
class MainWindow(QMainWindow):
    def __init__(self):
        ...
        # השורה הבאה מחליפה את ה-setStyleSheet שמחקת:
        ...
        self.btn_abs.setStyleSheet("background-color: #001F3F; color: white; font-weight: bold;")
    ...
    def _create_menubar(self):
        ...
        # --- תחילת העיצוב (QSS) ---
        menu_style = """
            /* עיצוב שורת התפריט הראשית (הפס למעלה) */
            QMenuBar {
                background-color: #DCE1E8; /* צבע רקע תואם למסך הראשי */
                color: #001F3F; /* טקסט בכחול כהה */
                font-weight: bold;
                font-size: 14px;
                border-bottom: 1px solid #8A9AAB; /* פס הפרדה עדין למטה */
            }

            /* עיצוב הכפתורים הראשיים (File, Position...) */
            QMenuBar::item {
                background-color: transparent;
                padding: 6px 14px; /* ריווח פנימי שנותן תחושה אוורירית */
                margin: 2px;
                border-radius: 4px; /* פינות מעוגלות */
            }

            /* כשמרחפים עם העכבר או לוחצים על כפתור בשורה הראשית */
            QMenuBar::item:selected {
                background-color: #8A9AAB;
                color: white;
            }

            /* עיצוב התפריט הנפתח (החלונית שקופצת למטה) */
            QMenu {
                background-color: #F8F9FA; /* רקע בהיר ונקי */
                color: #333333;
                border: 1px solid #8A9AAB;
                border-radius: 4px;
                padding: 4px 0px; /* קצת רווח למעלה ולמטה */
            }

            /* עיצוב השורות בתוך התפריט הנפתח */
            QMenu::item {
            ...
            /* כשמרחפים על שורה בתוך התפריט הנפתח */
            QMenu::item:selected {
                background-color: #001F3F; /* הדגשה בכחול כהה */
                color: white;
            }

            /* עיצוב קו ההפרדה בתוך התפריט (Separator) */
            QMenu::separator {
                height: 1px;
        ...
        # יצירת התפריטים עצמם (הקוד המקורי שלך)
        ...
        m_file.addSeparator()  # הקו הזה יעוצב לפי ה-separator ב-QSS
    ...
    def _on_refresh_clicked(self) -> None:
        ...
        self._last_cut_pixmap = None  # מחיקה מהזיכרון כדי למנוע הופעה מחדש ב-Resize
    ...
    def display_cut_result(self, pixmap: QPixmap, contours=None, is_looking_up=False, cut_z: float = None):
        ...
        for cnt in contours:
            poly = QPolygonF()
            for pt in cnt:
                # ההמרה מקואורדינטות 3D לפיקסלים עם קנה המידה המעודכן
                if is_looking_up:
    ...
    def open_angle_between_surfaces_window(self) -> None:
        ...
        self.abs_win = AngleBetweenSurfacesWindow(mesh, parent=None)
...
def convert_wrl_to_ply(wrl_path: str) -> str:
    """
    ממיר קובץ WRL ל-PLY.
    הייבוא נמצא כאן כדי לא להכביד על זמן העלייה של התוכנה.
    """
    import pymeshlab
    import os

    ms = pymeshlab.MeshSet()
    ms.load_new_mesh(wrl_path)

    base_name = os.path.splitext(wrl_path)[0]
    ply_path = f"{base_name}_converted.ply"
