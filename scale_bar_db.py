# artifact_app/viewer/scale_bar.py
# -*- coding: utf-8 -*-
...
# ------------------------------------------------------------
# MATLAB: scale_size_for_GUI (clone)
# ------------------------------------------------------------
def _scale_size_for_gui_matlab(s0: float) -> tuple[int, float]:
    """
    מחזיר:
      sl      = מספר מקטעים (2/3/5)
      scal_mm = אורך "יפה" במ"מ
    """
    sc = 1.0
    not_done = True
    s1 = 3
...
def _world_width_visible_mm(meta: dict) -> float:
    """
    רוחב עולם נראה במבט אורתוגרפי:
      world_height = 2*parallel_scale
...
def make_scale_pixmap(
    ...
    *,
    ...
    font_size: int | None = None,  # פרמטר חדש לגודל פונט
) -> QPixmap:
    ...
    # גודל פונט - אם לא צוין, משתמשים בגודל קטן יותר
    if font_size is not None:
        fs = max(6, int(font_size))
    else:
        # ברירת מחדל קטנה יותר: 20% מגובה התא במקום 35%
        fs = max(8, int(h_scale * 0.20))
