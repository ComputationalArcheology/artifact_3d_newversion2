...
class ProcessObjectWindow(QWidget):
    def __init__(self, mesh_aligned, mc_meta=None, ml_meta=None, parent=None, filename: str = ""):
        ...
        lbl_title.setStyleSheet("font-weight:bold; font-size:14px; background:#d0d7e2;")
        ...
        self.btn_menu.setStyleSheet("font-weight: bold; padding: 5px;")
    ...
    def update_measurements(self) -> None:
        ...
        if m.bind3box and m.bind3box.dims:
            ...
            lines.append(f"  Length (Height): {d['dz']:.1f}")
            lines.append(f"  Width:           {d['dx']:.1f}")
            lines.append(f"  Thickness:       {d['dy']:.1f}")
        ...
        lines.append("")
        lines.append(f"XZ Max Cord (Horiz): {m.profile_xz.max_chord:.1f}")
        if m.profile_xz.matlab_data:
            lines.append(f"XZ Max Cord (Vert):  {m.profile_xz.matlab_data.cordh:.1f}")

        lines.append(f"YZ Max Cord (Horiz): {m.profile_yz.max_chord:.1f}")
        if m.profile_yz.matlab_data:
            lines.append(f"YZ Max Cord (Vert):  {m.profile_yz.matlab_data.cordh:.1f}")
