...
class ManualMeasurementsWindow(QMainWindow):
    ...
    def _update_result_text(self, res: MeasureResult) -> None:
        if self._angle_visible:
            ...
            self.lbl_sub.setText(
                " | ".join(
                    [
                        f"2D: distance={res.distance_2d:.3f} mm,  angle w/ Z={ang2:.3f}°",
                        f"3D: distance={res.distance_3d:.3f} mm,  angle w/ Z={ang3:.3f}°",
                    ]
                )
            )
        else:
            self.lbl_sub.setText(
                " | ".join(
                    [
                        f"2D: distance={res.distance_2d:.3f} mm",
                        f"3D: distance={res.distance_3d:.3f} mm",
                    ]
                )
            )
    ...
    def _on_toggle_angle(self) -> None:
        ...
        if self._res is not None:
            ...
            if self._A is not None:
                self._draw_marker("A", self._A)
            if self._B is not None:
                self._draw_marker("B", self._B)
            self._draw_measurement_overlay(self._res)
            self._update_result_text(self._res)
    ...
    def _on_save_data(self) -> None:
        ...
        default_name = f"{self.object_name} - manual measurements.txt"
        ...
        with open(path, "a", encoding="utf-8") as f:
            f.write(f"{ts}\n")
            f.write(f"Manual measurements for: {self.object_name}\n")
            f.write(f"Positioning: {self.postype}\n")
            f.write(f"Point A: [{A[0]:.6f}  {A[1]:.6f}  {A[2]:.6f}]\n")
            f.write(f"Point B: [{B[0]:.6f}  {B[1]:.6f}  {B[2]:.6f}]\n")
            f.write(f"Distance (3D): {res.distance_3d:.6f} mm\n")
            f.write(f"Angle w/ Z (3D): {float(np.degrees(res.z_angle_3d_rad)):.6f} deg\n")
            f.write("-------------------------------------------------\n\n")
